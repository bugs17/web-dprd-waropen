  ...
  return (
    <div className="flex flex-wrap justify-center gap-3 px-6 pb-11 bg-[#231c26]">
        ...
        {data.length > 0 ? 
            ...
            data.map((item, index) => (
                <Card key={index} imgUrl={`/api/berita/image/${item.imageUrl}`} judul={item.judul} update={new Date(item.updatedAt).toLocaleString('id-ID', {day: 'numeric',month: 'long',year: 'numeric'})} urlBerita={`/berita/${item.slug}`} />
            ))
        :
        (
            <>
